...
const EditAffiliate = ({ item }) => {
  ...
  const [commissionRate, setCommissionRate] = useState(
    item.commissionRate ? item.commissionRate['$numberDecimal'] : 0
  );
  ...
  const submitForm = async (formData) => {
    ...
    try {
      const res = await axios.patch(
        `${process.env.BASE_URL}/affiliates/${item._id}`,
        formData,
        config
      );
      ...
      setTimeout(() => {
        setShowNoti(false);
        Router.push(`/affiliates/${item._id}`);
        setMessage('');
        setAlertType('');
      }, 2000);
    } catch (err) {
    ...
    <MainContent>
      ...
      >
        <div className="form-content">
          ...
          <FormGroup>
            <FormLabel htmlFor="phoneNumber">Só điện thoại</FormLabel>
            <FormInput
              type="text"
              placeholder="Tên"
              id="phoneNumber"
